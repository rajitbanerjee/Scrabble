...
class TileTest {
    ...
    void testTile() {
        ...
        if (t.getType() != 'Z') {
            fail("type accessor doesn't work");
        }

        // Try to set an invalid tile type
        try {
            t.setType('*');
            fail("type setter should check for invalid tile types");
        ...
        if (t.getPoints() != 10) {
            fail("points accessor doesn't work");
        }

        // Try to set invalid tile points
        try {
            t.setPoints(11);
            fail("point setter should allow maximum 10 points");
        } catch (Exception e) {
            // Test passed
        }
        ...
        try {
            t.setPoints(-1);
            System.out.println("point setter shouldn't allow negative points");
        } catch (Exception ignored) {
            // Test passed
        }
        ...
        if (!t.toString().equals("Z")) {
            fail("toString() doesn't work as expected.");
        }
